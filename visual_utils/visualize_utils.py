...
def visualize_pts(pts, fig=None, bgcolor=(1, 1, 1), fgcolor=(0.0, 0.0, 0.0),
                  show_intensity=False, size=(600, 600), draw_origin=False):
...
def draw_scenes(points, gt_boxes=None, gt_labels=None, ref_boxes=None, ref_scores=None, ref_labels=None):
    ...
    if gt_labels is not None and not isinstance(gt_labels, np.ndarray):
    ...
    if gt_boxes is not None:
        ...
        if gt_labels is None:
            ...
            for k in range(gt_labels.min(), gt_labels.max() + 1):
                print(gt_labels.min(), gt_labels.max() + 1)
                cur_color = tuple(box_colormap[k % len(box_colormap)])
                mask = (gt_labels == k)
                fig = draw_corners3d(corners3d[mask], fig=fig, color=cur_color, max_num=100)
